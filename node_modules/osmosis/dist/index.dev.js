...
var _Command = _interopRequireWildcard(require("../lib/Command.js"));

var _Queue = _interopRequireDefault(require("../lib/Queue.js"));

var _Request = _interopRequireDefault(require("../lib/Request.js"));

var _libxmljsDom = _interopRequireWildcard(require("libxmljs-dom"));
...
var toMB = function toMB(size, num) {
  return (size / 1024 / 1024).toFixed(num || 2) + 'Mb';
};

var extend = function extend(object, donor) {
  var key,
...
function Osmosis(url, params) {
  ...
  this.command = new _Command["default"](this);
  this.id = ++instanceId;
}
/**
 * @name options
...
Osmosis.config = Osmosis.prototype.config = function (option, value) {
...
Osmosis.prototype.request = function (url, opts, callback, tries) {
  ...
  (0, _Request["default"])(url.method, url, url.params, opts, tries, function (err, res, data) {
    var proxies = opts.proxies;
    self.queue.requests--;

    if ((res === undefined || res.statusCode !== 404) && proxies !== undefined) {
      self.command.error('proxy ' + (proxies.index + 1) + '/' + proxies.length + ' failed (' + opts.proxy + ')'); // remove the failing proxy
    ...
    if (err !== null && ++tries < opts.tries) {
      ...
      if (self.opts.log === true) {
        self.command.error(err + ', retrying ' + url.href + ' (' + (tries + 1) + '/' + opts.tries + ')');
      }
    } else {
    ...
    if (self.opts.log === true) {
      self.command.log('[redirect] ' + href + ' -> ' + new_url);
    }
  });
};
/**
...
Osmosis.prototype.queueRequest = function (url, opts, callback, tries) {
  if (tries === undefined) {
    tries = 0;
  }
...
Osmosis.prototype.parse = function (data, opts) {
   ...
   */
  var document = (0, _libxmljsDom.parseHtml)(data, opts);
...
Osmosis.prototype.resources = function () {
  var mem = process.memoryUsage(),
      ...
      libxml_mem = (0, _libxmljsDom.memoryUsage)(),
      nodes = (0, _libxmljsDom.nodeCount)();
  ...
  this.command.debug('stack: ' + this.queue.count + ', ' + 'requests: ' + this.requests + ' (' + this.queue.requests + ' queued), ' + 'RAM: ' + toMB(mem.rss) + ' (' + memDiff + '), ' + 'libxml: ' + (libxml_mem / mem.rss * 100).toFixed(1) + '% (' + nodes + ' nodes), ' + 'heap: ' + (mem.heapUsed / mem.heapTotal * 100).toFixed(0) + '% of ' + toMB(mem.heapTotal));
  memoryUsage = mem.rss;
};
/**
...
Object.keys(_Command.prototype).forEach(function (name) {
...
_libxmljsDom.Document.prototype.findXPath = _libxmljsDom.Document.prototype.find;
_libxmljsDom.Element.prototype.findXPath = _libxmljsDom.Element.prototype.find;

_libxmljsDom.Document.prototype.find = function (selector, cache) {
  return this.root().find(selector, cache);
};

_libxmljsDom.Element.prototype.find = function (selector) {
  if (selector.charAt(1) === '/' || selector.charAt(0) === '/' || selector.charAt(0) === '(') {
    return this.findXPath(selector);
  } else if (cachedSelectors[selector] === undefined) {
    cachedSelectors[selector] = (0, _libxmljsDom.css2xpath)(selector);
  }
...
Osmosis.libxmljs = _libxmljsDom["default"];
var _default = Osmosis;
